...
class SimpleCalculator : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val buttons = listOf(
            ...
            Pair(R.id.btnOpenParen, "("), Pair(R.id.btnCloseParen, ")"), Pair(R.id.btnPower, "^")
        )
    ...
    // zmiana tokenów w notacji infixowej do tokenów w odwrotnej notacji polskiej
            ...
            when {
                ...
                token in listOf("sin", "cos", "tan", "ln", "sqrt", "log") -> {
                    ...
                    val result = when (token) {
                        ...
                        "ln" -> if (a > 0) ln(a) else throw ArithmeticException("for ln(x) when x <= 0")
                        "sqrt" -> if (a >= 0) sqrt(a) else throw ArithmeticException("for sqrt(x) when x < 0")
                        "log" -> if (a > 0) log10(a) else throw ArithmeticException("for log(x) when x <= 0")
